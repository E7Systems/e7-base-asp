--create script for RSFF database
--requires sql 2005 or newer
--must be logged into SQL Manager with Windows Authentication as this scripts needs to write to Master db

--any errors in this script should be fatal
SET XACT_ABORT ON
...
--if sql login 'rsffdb' does not exist then create it.  if it exists, then use it.
IF(NOT EXISTS (select * from dbo.syslogins WHERE name = 'rsffdb'))
	--note:  the [rsff] database must exist or sp_addlogin will fail
	--when it tries to set the default database (@defdb =...)
	EXEC master.dbo.sp_addlogin @loginame = N'rsffdb', @passwd = N'firei$bad', @defdb = N'rsff'
GO

--make the rsffdb user the owner of the rsff database
USE [rsff]
GO

EXEC dbo.sp_changedbowner 'rsffdb'
GO
